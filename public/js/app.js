...
var genshin = {
  ...
  methods: {
    toResultView: function toResultView() {
      this.view = 'result';
    },
    ...
    toSetFocus: function toSetFocus() {
      this.set_focus = true;
    },
    outSetFocus: function outSetFocus() {
      this.set_focus = false;
    },
    toSetOptionalPool: function toSetOptionalPool() {
      this.set_optional_pool = true;
    },
    outSetOptionalPool: function outSetOptionalPool() {
    ...
    outSetting: function outSetting() {
      this.setting = false;
    },
    ...
    single: function single() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/wish', {
        'wish_click': this.wish_click,
        'selected': this.selected
      }).then(function (response) {
        _this.view = 'result'; //result介面

        _this.video_end = false; //播放影片

        var synk_info = response['data']['synk_info'];

        _this.Synk(synk_info); //同步


        _this.results = response['data']['results'];
        _this.wish_item_img[0] = {
          //style設定
          "background-image": "url(" + _this.results[0]['img'] + ")",
          "background-size": _this.results[0]["kind"] == "wp" ? "inherit" : "contain"
        };
        _this.video = response['data']['video']; //video
      }, function () {
        alert('失敗');
      });
    },
    ten: function ten() {
      ...
      axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/wish', {
        ...
        _this2.view = 'result';
        _this2.video_end = false;
        var synk_info = response['data']['synk_info'];

        _this2.Synk(synk_info);

        _this2.video = response['data']['video'];
        var r = [[], [], [], [], [], [], [], [], [], []];
        var j = 0;
        var done = {};
        ...
        for (var i = 0; i < 10; i++) {
          ...
          if (!Object.keys(done).includes(cur)) {
            //當key不含cur，就在done[cur]處新增j，表示cur是第幾個
            done[cur] = j;
            j += 1;
          }
        ...
        for (var i = 0; i < j; i++) {
          ...
          r[i].forEach(function (a) {
            _this2.results.push(a);

            _this2.results[_this2.results.length - 1]['count'] = r[i].length; //在當前results處新增count(數量)
          });
        }

        ;
        _this2.results = _this2.results.sort(_this2.WishSort); //排序

        for (var i = 0; i < 10; i++) {
          _this2.wish_item_img[i] = {
            'background-image': 'url(' + _this2.results[i]['img'] + ')',
            'background-size': _this2.results[i]['kind'] == 'cr' ? 'cover' : 'contain',
    ...
    WishSort: function WishSort(a, b) {
      var _this3 = this;

      if (a.star > b.star) return -1;else if (a.star < b.star) return 1;else {
        if (this.selected == 0 || this.selected == 1) {
          var isup = false;
          Object.keys(this.pool_info).forEach(function (cato) {
            if (cato.includes('up')) {
              if (_this3.pool_info[cato].includes(b.name)) isup = true;
            }
          });
          if (isup) return 1;else {
            if (a.kind > b.kind) return 1;else if (a.kind < b.kind) return -1;else return 0;
          }
        } else {
          if (a.kind > b.kind) return 1;else if (a.kind < b.kind) return -1;else return 0;
        }
    ...
    FocusClick: function FocusClick(n) {
      this.focus_clicked = n;
    },
    SetFocus: function SetFocus() {
      var _this4 = this;

      axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/set_focus', {
        'focus_item': this.wp_up_info[this.focus_clicked]['name'],
        'action': 'set'
      }).then(function (response) {
        _this4.focus_info = response['data']['focus_info'];
        _this4.focus_selected = _this4.focus_clicked;
      }, function () {
        alert('失敗');
      });
    },
    UnsetFocus: function UnsetFocus() {
      var _this5 = this;

      axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/set_focus', {
        'action': 'unset'
      }).then(function (response) {
        _this5.focus_info = response['data']['focus_info'];
      }, function () {
        alert('失敗');
      });
    },
    ResetCur: function ResetCur() {
      axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/reset', {
        'action': 'reset_cur'
      }).then(function (response) {
        window.location.reload();
      }, function () {
    ...
    OptSelect: function OptSelect(name, group) {
      var _this6 = this;
      ...
      switch (name) {
        ...
        case 'select_all':
          Object.keys(this.optional_pool).forEach(function (i) {
            _this6.optional_pool[i] = JSON.parse(JSON.stringify(_this6.all_cr_wp[i]));
          });
          break;

        case 'unselect_all':
          Object.keys(this.optional_pool).forEach(function (i) {
            _this6.optional_pool[i] = [];
          });
          break;
        ...
        default:
          if (this.optional_pool[group].includes(name)) this.optional_pool[group].splice(this.optional_pool[group].indexOf(name), 1);else this.optional_pool[group].push(name);
          break;
      }

      Object.keys(this.all_cr_wp).forEach(function (i) {
        _this6.selection[i] = true;

        for (var a = 0; a < _this6.all_cr_wp[i].length; a++) {
          if (!_this6.optional_pool[i].includes(_this6.all_cr_wp[i][a])) {
            _this6.selection[i] = false;
          }
        }
      });
      ...
      ;
      Object.values(this.optional_pool).forEach(function (i) {
        if (i.length != 0) _this6.selection['all_un'] = false;
      });
    },
    OptSave: function OptSave() {
      var _this7 = this;

      if (this.selection['all_un']) {
        alert('請至少選擇一項！！！');
      } else {
        axios__WEBPACK_IMPORTED_MODULE_0___default().post('genshin/set_optional_pool', {
          'optional_pool': this.optional_pool
        }).then(function (response) {
          _this7.old_optional_pool = JSON.parse(JSON.stringify(_this7.optional_pool));

          _this7.Synk(response['data']['synk_info']);

          _this7.outSetOptionalPool();
        }, function () {
          alert('失敗');
        });
      }
    },
    HistoryFilter: function HistoryFilter() {
      var _this8 = this;

      var inv = JSON.parse(JSON.stringify(this.old_inventory));

      if (this.history_star != 'all') {
        Object.keys(inv).forEach(function (i) {
          inv[i] = inv[i].filter(function (object) {
            return object['star'] == _this8.history_star;
          });
        });
      }
      ...
      if (this.history_kind != 'all') {
        Object.keys(inv).forEach(function (i) {
          inv[i] = inv[i].filter(function (object) {
            return object['kind'] == _this8.history_kind;
          });
        });
      }
      ...
      if (this.history_order == 'desc') {
        Object.keys(inv).forEach(function (i) {
          Object.keys(inv[i]).forEach(function (j) {
            inv[i] = inv[i].sort(function (a, b) {
              if (a[_this8.history_order_type] > b[_this8.history_order_type]) return -1;else if (a[_this8.history_order_type] < b[_this8.history_order_type]) return 1;else return 0;
            });
          });
        });
      } else {
    ...
    NextPage: function NextPage() {
      if (this.page[this.history_selected] + 1 == this.inventory[this.history_selected].length) {} else {
...
(0,vue__WEBPACK_IMPORTED_MODULE_1__.createApp)(genshin).mount('#simu');
